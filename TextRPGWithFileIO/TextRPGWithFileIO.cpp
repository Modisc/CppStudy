...
int main()
{
	...
	while (gameMode <= GM_NONE || gameMode > GM_END)
	{
		system("cls");
		cout << "1. 새로하기" << endl;
		cout << "2. 이어하기" << endl;
	...
	LoadStore(storeWeapon, storeArmor);
...
bool LoadPlayer(Player* player)
{
	...
	fopen_s(&file, "Player.ply", "rb");
	if (file)
	{
		// 플레이어 이름 및 직업정보 읽어오기
		fread(player->name, 1, NAME_SIZE, file);
		fread(&player->job, sizeof(JOB), 1, file);
		fread(player->jobName, 1, NAME_SIZE, file);

		// 공격력
		fread(&player->attackMax, sizeof(int), 1, file);
		fread(&player->attackMin, sizeof(int), 1, file);

		// 방어력
		fread(&player->armorMax, sizeof(int), 1, file);
		fread(&player->armorMin, sizeof(int), 1, file);

		// 체력
		fread(&player->hp, sizeof(int), 1, file);
		fread(&player->hpMax, sizeof(int), 1, file);

		// 마나
		fread(&player->mp, sizeof(int), 1, file);
		fread(&player->mpMax, sizeof(int), 1, file);

		// 경험치와 레벨
		fread(&player->exp, sizeof(int), 1, file);
		fread(&player->level, sizeof(int), 1, file);

		// 무기 아이템 착용여부
		fread(&player->equipCheck[EQ_WEAPON], sizeof(bool), 1, file);

		// 만약 저장할 때 무기를 차고 있었다면
		// 해당 무기 정보도 함께 저장된다.
		// 따라서 무기를 차고있을 경우
		// 무기도 읽어와야 한다.
		if (player->equipCheck[EQ_WEAPON])
		{
			fread(&player->currEquipItem[EQ_WEAPON], sizeof(Item), 1, file);
		}

		// 방어구 아이템 착용여부
		fread(&player->equipCheck[EQ_ARMOR], sizeof(bool), 1, file);
		if (player->equipCheck[EQ_ARMOR])
		{
			fread(&player->currEquipItem[EQ_ARMOR], sizeof(Item), 1, file);
		}

		// 골드를 읽어온다.
		fread(&player->inventory.gold, sizeof(int), 1, file);

		// 인벤토리 아이템 개수를 읽어온다
		fread(&player->inventory.itemCount, sizeof(int), 1, file);

		// 읽어온 아이템 수만큼 인벤토리에 아이템정보를 읽어온다.
		fread(player->inventory.item, sizeof(Item), player->inventory.itemCount, file);
...
bool SavePlayer(Player* player)
{
	...
	fopen_s(&file, "player.ply", "wb");
	if (file)
	{
		// 플레이어 이름 및 직업정보 저장하기
		fwrite(player->name, 1, NAME_SIZE, file);
		fwrite(&player->job, sizeof(JOB), 1, file);
		fwrite(player->jobName, 1, NAME_SIZE, file);

		// 공격력
		fwrite(&player->attackMax, sizeof(int), 1, file);
		fwrite(&player->attackMin, sizeof(int), 1, file);

		// 방어력
		fwrite(&player->armorMax, sizeof(int), 1, file);
		fwrite(&player->armorMin, sizeof(int), 1, file);

		// 체력
		fwrite(&player->hp, sizeof(int), 1, file);
		fwrite(&player->hpMax, sizeof(int), 1, file);

		// 마나
		fwrite(&player->mp, sizeof(int), 1, file);
		fwrite(&player->mpMax, sizeof(int), 1, file);

		// 경험치와 레벨
		fwrite(&player->exp, sizeof(int), 1, file);
		fwrite(&player->level, sizeof(int), 1, file);

		// 무기 아이템 착용여부
		fwrite(&player->equipCheck[EQ_WEAPON], sizeof(bool), 1, file);
		...
		if (player->equipCheck[EQ_WEAPON])
		{
			fwrite(&player->currEquipItem[EQ_WEAPON], sizeof(Item), 1, file);
		}

		// 방어구 아이템 착용여부
		fwrite(&player->equipCheck[EQ_ARMOR], sizeof(bool), 1, file);
		if (player->equipCheck[EQ_ARMOR])
		{
			fwrite(&player->currEquipItem[EQ_ARMOR], sizeof(Item), 1, file);
		}

		// 골드를 저장하기
		fwrite(&player->inventory.gold, sizeof(int), 1, file);

		// 인벤토리 아이템 개수를 저장하기
		fwrite(&player->inventory.itemCount, sizeof(int), 1, file);

		// 읽어온 아이템 수만큼 인벤토리에 아이템정보를 저장하기
		fwrite(player->inventory.item, sizeof(Item), player->inventory.itemCount, file);
...
void SetMonster(Monster* monsterArr)
{
		...
		if (file)
		{
			fread(monsterArr, sizeof(Monster), MT_BACK - 1, file);
			fclose(file);
		}
